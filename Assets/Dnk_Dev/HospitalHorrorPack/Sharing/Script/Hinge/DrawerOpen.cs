...
public class DrawerOpen : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        drawer = transform.GetChild(0).GetComponent<GetItem>();
    }
    ...
    private void TryEvent()
    {
            ...
            if (playerHand.hitinfo.transform == transform.GetChild(0)) // 에임이 물체에 있는 상태에서
            {
                ...
                if (OVRInput.GetDown(OVRInput.Button.One)) //키 누르면(한 번만 실행할 부분)
                {
                    ...
                    if (drawer.items.Length > 0)
                    {
                        ...
                        {
                            if (drawer.items[i] != null) // 안에 물건이 있으면
                            {
                                drawer.items[i].transform.parent = transform.GetChild(0); //물건 같이 이동
                            }
                        }
                    }*/
                }
        ...
        {
            if (transform.position == new Vector3(initPos.x, initPos.y, initPos.z))
            {
                if (drawer.items.Length > 0)
                {
                    for (int i = 0; i < drawer.items.Length; i++)
                    {
                        if (drawer.items[i] != null)
                        {
                            drawer.items[i].SetActive(true); //안에 있는 물건 나타나고
                        }
                    }
                }
            }
            ...
            var newPos = Vector3.MoveTowards
                (transform.position, new Vector3(initPos.x, initPos.y, initPos.z - 0.5f), 0.02f);
        ...
        else// 닫히면
        {
            if (transform.position == new Vector3(initPos.x, initPos.y, initPos.z))
            {
                if (drawer.items.Length > 0)
                {
                    for (int i = 0; i < drawer.items.Length; i++)
                    {
                        if (drawer.items[i] != null)
                        {
                            drawer.items[i].SetActive(false); //안에 있는 물건 사라짐
                        }
                    }
                }
            }
            ...
            var newPos = Vector3.MoveTowards
                (transform.position, new Vector3(initPos.x, initPos.y, initPos.z), 0.02f);
